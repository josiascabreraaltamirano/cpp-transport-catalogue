    ...
    namespace output {
        ...
        namespace detail {
            ...
            RequestInfo ParseRequest(std::string_view string) {
                ...
                return new_start != raw_route_name.npos ? 
                RequestInfo{type, raw_route_name.substr(new_start, raw_route_name.find_last_not_of(' ') + 1 - new_start)} : 
        ...
        void ParseAndPrintStat(const database::TransportCatalogue& transport_catalogue, std::string_view request,
            ...
            if (request_info.type.size() == 3) {
                ...
                std::setprecision(6) << " unique stops, "sv << result.route_length << " route length"sv);
                output << '\n';
                //finish    
                return;
            }
